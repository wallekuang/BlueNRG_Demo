      ...
      if(end != 1) {
        ...
        while(j < end) {
          ...
          if (device_role == SLAVE_ROLE) {
            while(aci_gatt_update_char_value_ext(connection_handle,chatServHandle,TXCharHandle,1,len,0,len,(uint8_t *)cmd+j)==BLE_STATUS_INSUFFICIENT_RESOURCES) { 
            ...
            while(aci_gatt_write_without_resp(connection_handle, rx_handle+1, len, (uint8_t *)cmd+j)==BLE_STATUS_INSUFFICIENT_RESOURCES) { 
              BTLE_StackTick();
              // Radio is busy (buffer full).
              if(Timer_Expired(&t))
                break;
