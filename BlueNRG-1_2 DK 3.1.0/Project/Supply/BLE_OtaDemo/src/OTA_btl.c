...
#ifdef ST_OTA_LOWER_APPLICATION
static uint32_t const OTA_information[4]__attribute__((at(APP_LOWER_ADDRESS+OTA_LOWER_OR_HIGHER_FLAG_ADDR_POS))) = {APP_LOWER_ADDRESS};
#else
static uint32_t const OTA_information[4]__attribute__((at(APP_HIGHER_ADDRESS+OTA_LOWER_OR_HIGHER_FLAG_ADDR_POS))) = {APP_HIGHER_ADDRESS};
#endif
#endif
#endif
  // other
...
//@brief incase if OTA not completed ,and then try to OTA again.
void clearn_up_OTA_data(void)
{
		detected_error = 0;
    write_counter = 0;
		expectedSeqNum = 0;
		last_written_receivedSequence = 0;
		notification.replyCounter = 0;
    notification.errCode = 0; 
		client_DLE_ATT_MTU = 0;
}
...
tBleStatus OTA_Add_Btl_Service(void)
{
    tBleStatus ret;
    uint8_t uuid[16];
    uint8_t imageinfo[9] = {BLE_OTA_DLE_SERVER_VERSION,0,0,0,0,0,0,0,0}; //BLE OTA DLE server version 
    ...
    ret = aci_gatt_update_char_value_ext(conn_handle,btlServHandle, btlNewImageCharHandle, 1,9, 0, 9,(uint8_t *) imageinfo);
    ...
    ret =  aci_gatt_add_char(btlServHandle, UUID_TYPE_128, &ota_char_uuid, IMAGE_CONTENT_SIZE, CHAR_PROP_READ|CHAR_PROP_WRITE|CHAR_PROP_WRITE_WITHOUT_RESP, ATTR_PERMISSION_NONE, GATT_NOTIFY_READ_REQ_AND_WAIT_FOR_APPL_RESP | GATT_NOTIFY_ATTRIBUTE_WRITE,
                             16, 1, &btlNewImageTUContentCharHandle); 
    if (ret != BLE_STATUS_SUCCESS) goto fail;
    ...
    ret = aci_gatt_update_char_value_ext(conn_handle,btlServHandle, btlImageCharHandle, 1,8, 0, 8,(uint8_t *) currentImageInfos);
    if (ret != BLE_STATUS_SUCCESS)
    {
      PRINTF("Error while updating characteristic.\n");
      return BLE_STATUS_ERROR;
...
static void OTA_Check_Update_Error_Condition(uint8_t notification_range)
{
  ...
  {              
  	 PRINTF("%s\n", __func__);
     ret = aci_gatt_update_char_value_ext(conn_handle,btlServHandle, btlExpectedImageTUSeqNumberCharHandle, 1,4, 0, 4,(uint8_t*)&notification);
     if (ret != BLE_STATUS_SUCCESS)
       PRINTF("Error while updating  characteristic.\n");
                          ...
                          uint8_t *att_data)
{
    ...
    conn_handle = connection_handle;
		...
		PRINTF("attr_handle:%x   data_length:%x \n",	attr_handle,data_length);
    ...
    else if (attr_handle == (btlNewImageTUContentCharHandle + 1))
    {
			if(data_length > 10)
			{
					...
					PRINTF("...");
					for(int i=data_length-10;i<data_length;i++){
							PRINTF("%x ", att_data[i]);
					}
					PRINTF("\n");
			}
       ...
       //else 
			 if (bufPointer < BUFPOINTER_LIMIT(ota_att_mtu_size)){
          ...
          */
          ...
          if (checksum == att_data[0]){
             ...
             { 
             		PRINTF("(expectedSeqNum == receivedSeqNum) == %x \n", receivedSeqNum);
...
void OTA_Radio_Activity(uint32_t Next_State_SysTime)
{
  ...
  if (ota_write_data) 
  {
    ...
    if (HAL_VTimerDiff_ms_sysT32(Next_State_SysTime, HAL_VTimerGetCurrentTime_sysT32()) > OTA_WRITE_GUARD_TIME(ota_att_mtu_size)) 
    {
        ota_write_data = 0; 
        OTA_Write_Data(); 
    }
                                     ...
                                     uint16_t Att_MTU)
{
  ...
  aci_l2cap_connection_parameter_update_req(Connection_Handle, (uint16_t) OTA_EXT_LE_L2CAP_CONN_INTERVAL(ota_att_mtu_size), (uint16_t) OTA_EXT_LE_L2CAP_CONN_INTERVAL(ota_att_mtu_size), 0, 0xC80);
}
