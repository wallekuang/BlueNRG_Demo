...
#define BLE_TEST_PAGE_ADDRESS        (_MEMORY_FLASH_END_+1 - 4*_MEMORY_BYTES_PER_PAGE_)
uint8_t data_to_write[] = "This is a flash access test......";



static void *write_done(ErrorStatus sta,void *param)
{
		printf("%s sta: %s  \n", __func__, ((sta==SUCCESS)?"success":"failure"));
		static uint8_t temp[sizeof(data_to_write)];
		memset(temp,0,sizeof(data_to_write));
		printf("read data of written:\n");
		for(int i=0;i<sizeof(temp);i++)
		{
				temp[i] = FLASH_ReadByte(BLE_TEST_PAGE_ADDRESS+i);
		}
		printf("%s \n",temp);
		return NULL;
}

static void *erase_done(ErrorStatus sta,void *param)
{
		printf("%s sta: %s  \n", __func__, ((sta==SUCCESS)?"success":"failure"));
		flash_list_program_write(BLE_TEST_PAGE_ADDRESS,data_to_write,sizeof(data_to_write),write_done,NULL);
		return NULL;
}

static void test_flash_list(void)
{
		uint16_t page = (BLE_TEST_PAGE_ADDRESS - _MEMORY_FLASH_BEGIN_)/_MEMORY_BYTES_PER_PAGE_;
		flash_list_erase_page(page, erase_done, NULL);
...
{
		static uint8_t has_writen = 0;
		uint32_t cur_time = HAL_VTimerGetCurrentTime_sysT32();
		// 在程序运行10s 后，模拟一个写操作
		if( (cur_time  > 409600*10) && !has_writen){
				has_writen = 1;
				test_flash_list();
		}
}
...
int main(void)
{
  ...
  if (ret != BLE_STATUS_SUCCESS) {
	...
	//test_flash_api_burst_write();
	flash_operate_mutex_BEL_event_init();
